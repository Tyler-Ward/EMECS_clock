...
void drawbigchar(uint8_t *buff, uint8_t x, uint8_t line, uint8_t c){
	uint8_t temp_buffer;
	uint8_t byte1,byte2,byte3;
	
	for(uint8_t i=0; i<5; i++ ){
		temp_buffer = pgm_read_byte(font+(c*5)+i);
		byte1 = 0;
		byte1 |= (temp_buffer & 0x80)|((temp_buffer & 0x80)>>1)|((temp_buffer & 0x80)>>2);
		byte1 |= ((temp_buffer & 0x40)>>2)|((temp_buffer & 0x40)>>3)|((temp_buffer & 0x40)>>4);
		byte1 |= ((temp_buffer & 0x20)>>4)|((temp_buffer & 0x20)>>5);
		byte2 = 0;
		byte2 |= ((temp_buffer & 0x20)<<2)|((temp_buffer & 0x10)<<2)|((temp_buffer & 0x10)<<1);
		byte2 |= (temp_buffer & 0x10)|(temp_buffer & 0x08)|((temp_buffer & 0x08)>>1);
		byte2 |= ((temp_buffer & 0x08)>>2)|((temp_buffer & 0x04)>>2);
		byte3 = 0;
		byte3 |= ((temp_buffer & 0x04)<<5)|((temp_buffer & 0x04)<<4)|((temp_buffer & 0x02)<<4);
		byte3 |= ((temp_buffer & 0x02)<<3)|((temp_buffer & 0x02)<<2)|((temp_buffer & 0x01)<<2);
		byte3 |= ((temp_buffer & 0x01)<<1)|(temp_buffer & 0x01);
		buff[x + (line*128)] = byte1;
		buff[x + (line*128) + 1] = byte1;
		buff[x + (line*128) + 2] = byte1;
		buff[x + ((line+1)*128)] = byte2;
		buff[x + ((line+1)*128) + 1] = byte2;
		buff[x + ((line+1)*128) + 2] = byte2;
		buff[x + ((line+2)*128)] = byte3;
		buff[x + ((line+2)*128) + 1] = byte3;
		buff[x + ((line+2)*128) + 2] = byte3;
		x+=3;
	}
}
